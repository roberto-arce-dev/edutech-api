# EduTech Innovators API

## Descripción

Usuario único con roles ESTUDIANTE/INSTRUCTOR, cada uno con su profile
...
- **EstudianteProfile** (rol: ESTUDIANTE)
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/edutech_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/edutech_db
...
**Rol asociado:** `ESTUDIANTE`

**Endpoints disponibles:**

#### Obtener mi perfil
```bash
GET /api/estudiante-profile/me
...
```bash
GET /api/estudiante-profile/{userId}
Authorization: Bearer {token_admin}
```
...
#### Obtener perfil por userId (Admin)
```bash
GET /api/instructor-profile/{userId}
Authorization: Bearer {token_admin}
```
...
## 📸 Crear Cursos con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un curso. Puedes:
- ✅ Crear el curso SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el Curso sin imagen
...

{
  ...
  "descripcion": "Curso completo de programación en Python",
  "duracion": 40,
  "nivel": "Principiante",
  "precio": 199.0
}
...
**💡 Nota:** Guarda el `_id` del curso creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al Curso

```bash
POST /api/curso/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del curso creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload Curso Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3001/api/curso/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu curso ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3001/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3001/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
# 2. Crear Curso
PRODUCTO_ID=$(curl -X POST http://localhost:3001/api/curso \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "Introducción a Python",   "descripcion": "Curso completo de programación en Python",   "duracion": 40,   "nivel": "Principiante",   "precio": 199.0 }' \
  | jq -r '.data._id')

echo "Curso creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3001/api/curso/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3001/uploads/`
...
```bash
curl -X GET http://localhost:3001/api/estudiante-profile/me \
...
```bash
curl -X PUT http://localhost:3001/api/estudiante-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `edutech-api.postman_collection.json`
...
│   │
│   ├── estudiante-profile/  # Profile ESTUDIANTE
│   │   ├── dto/
│   │   ├── schemas/
│   │   ├── estudiante-profile.controller.ts
│   │   ├── estudiante-profile.service.ts
│   │   └── estudiante-profile.module.ts
│   │
...
├── edutech-api.postman_collection.json
└── package.json
```
